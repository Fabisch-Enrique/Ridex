defmodule RidexWeb.AuthenticationControllerTest do
  ...
  describe "POST /api/authenticate" do
    test "returns OK with token", %{conn: conn} do
      body =
        conn
        |> post("/api/authenticate", %{"phone" => "+254792874627", "type" => "rider"})
        |> json_response(200)
    ...
    test "creates user", %{conn: conn} do
      body =
        conn
        |> post("/api/authenticate", %{"phone" => "+254701161418", "type" => "rider"})
        |> json_response(200)
    ...
    test "returns 400 with wrong user type", %{conn: conn} do
      conn
      |> post("/api/authenticate", %{"phone" => "+254792874627", "type" => "something"})
      |> json_response(400)
    end
  end
end
